...
class MovieListViewModelTests: XCTestCase {
    //    func testError() {
    //        let vm = MovieListViewModel(service: MockMovieService.error)
    //        let scheduler = TestScheduler()
    //
    //        let search = scheduler.createColdObservable([.next(25, "test")]).asDriver(onErrorJustReturn: nil)
    //        vm.bindSearch(input: search.asDriver())
